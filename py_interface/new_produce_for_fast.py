...
def _sample_info_add2(ch, start, end, name, editor_id, add_info={}):
    ...
    try:
        ...
        if ret[ 0 ] :
            url_req('http://192.168.0.153:10023/audio_detect', post_str= 'channel=%s&s_time=%s&e_time=%s&id=%s' %( ch , start, end ,ret[ 1 ]  )  , timeout_sec = 1 , try_count = 2 )
...
def audio_detect_state_by_channel(self):
    ...
    return 
    self.write( url_req( 'http://192.168.0.153:10023/audio_detect_state_by_channel?channel=%s&date=%s' %( ch ,day  )  ,timeout_sec = 1 , try_count = 2) )
