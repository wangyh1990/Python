...
def get_group_edit_error(ch_date, ch_date_end, check_date, group_id, status, check_date_end, jd_status, auther, channel, error_level,
    ...
    res = list( db.query(select_sql, "db_qc") )
    ...
    res_ch_man_item = {}
    if res:
        #ct_max = max([x['create_time'] for x in res])
        #ct_min = min([x['create_time'] for x in res])
        start_1 = res[0]["start"]
        start_2 = res[-1]["start"]
        kj_ch_list = []
        for row in res:
        ...
        if kj_ch_list:
            fzr_data = {
                ...
                "dateList": get_day_list(ts_to_time_str(start_2, "%Y-%m-%d"), ts_to_time_str(start_1, "%Y-%m-%d"))
                #"dateList": get_day_list(str(ct_min)[:10], str(ct_max)[:10])
            }
...
def edit_error_submit(self, channel_id, start, end):
    ...
    gp = db.query("SELECT user_group FROM `epg_status` where channel = '%s' and date = '%s'" % (channel_id, ts_to_time_str(start, time_format="%Y-%m-%d")), "db_produce_info")[0]["user_group"]
    ...
    # 通知全剪
    if desc != "质检完成":
        parse_allcut(channel_id, start, end, err_count, source, auther, desc)
