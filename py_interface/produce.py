...
def summary_clean( res ):
    long_ad = list( filter( lambda x :x[ 'end' ]  - x[ 'start' ] >= 300 , res ) )
    short_ad = list( filter( lambda x :x[ 'end' ]  - x[ 'start' ] < 300 , res ) )
    for x in short_ad :
        for y in long_ad:
            if int( x[ 'start' ] ) == int( y[ 'start' ] )  :
                x[ 'remove' ] =  1
                break
            elif int( x[ 'end' ] ) == int( y[ 'end' ] )  :
                x[ 'remove' ] =  1
                break
    short_ad  = list(  filter( lambda x : 'remove' not in x , short_ad ) )
    return sorted(  long_ad + short_ad ,key = lambda x :x[ 'start'] )

def summary_get( ch , start , end , times =  -1 ):
    end = min( end , start + 86400 )
    res = db.query( "select start   , end  ,ad_uuid uuid ,score  , create_time from summary where channel = '%s' and start between %s and %s and tag in ( 0,1,3 , 4 ) order by start "% ( ch , start ,end ) , "db" ) 
    for i in res : 
        if -1 != times : 
            i[ "start" ] = int( i[ "start" ] * times  )
            i[ "end" ] =  int( i[ "end" ] * times  )
        i[ "create_time" ] = str(  i[ 'create_time' ].strftime( '%Y-%m-%d %H:%M:%S' ) ) 
    return res
