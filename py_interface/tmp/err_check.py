...
def get_group_edit_error(ch_date, ch_date_end, check_date, group_id, status, check_date_end, jd_status, auther, channel, error_level,
    ...
    if channel == "" and area == "":
        ...
        resp_channel_list = js_load_req("http://172.16.198.126/manage/index/getChannelInfoByName?name={}&area={}".format(quote(channel), area))["dataList"]
    ...
    select_sql = "select id, channel, end, status, err_count count, start, start check_date, auther submit_user, gp, create_time, sys_err, `desc`, error_level level, admin_judge, entry_clerk, source from t_edit_err_info %s order by start" % where_sql
    ...
    if res:
        ...
        if kj_ch_list:
            ...
            res_ch_man_list = js_load_req("http://d.hz-data.com/Open/CutTask/fzr", post_str=json.dumps(fzr_data))["dataList"]
