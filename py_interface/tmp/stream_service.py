...
def range_switch_confirm( data_id  , confirm_status ) :
    ...
    def info_update() :
        if 1 == int(  confirm_status):
            ...
            url_req( "http://47.96.182.117/index/rewriteGatherChannel", post_str = 'channel=%s&date=%s' % ( res[ "channel_inner_id" ] , ts_to_time_str( res[ "start" ] , "%Y-%m-%d" ) ) )
            url_req( "http://47.96.182.117/index/rewriteGatherChannel", post_str = 'channel=%s&date=%s' % ( res[ "channel_inner_id" ] , ts_to_time_str( res[ "end" ] , "%Y-%m-%d" ) ) )
