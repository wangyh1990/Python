...
handler_list = [
    ...
    (r'/edit_task_exists/(?P<channel_id>.*)/(?P<start>.*)/(?P<end>.*)/', mk_req_handler(get=lambda self, channel_id, start, end: ret_write(self, 0 if _edit_task_exists(channel_id, int(start), int(end)) else 1))),
