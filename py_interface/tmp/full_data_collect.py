...
def get_tv_bro_list():
    bro_list =  [ i[ "channel" ] for i in  json.loads( url_req( "http://47.96.182.117/index/getChannelInfoById?type=2" ) )[ "dataList" ] ]
    tv_list =  [ i[ "channel" ] for i in json.loads( url_req( "http://47.96.182.117/index/getChannelInfoById?type=1" ) )[ "dataList" ] ]
...
def ch_detail_rep_get():
    ...
    all_ch_list = []
    tmp =  json.loads( url_req( "http://47.96.182.117/index/getAllChannel?is_use=0" ) )[ "dataList" ]
    for i in tmp : 
        i[ "confirmed" ] = "Y"
        i[ "outer_collected" ] = "N"
    ...
    tmp =  json.loads( url_req( "http://47.96.182.117/index/getAllChannel?is_use=1" ) )[ "dataList" ]
    for i in tmp : 
        i[ "confirmed" ] = "Y"
        i[ "outer_collected" ] = "Y"
    ...
    tmp =  json.loads( url_req( "http://47.96.182.117/index/getAllChannel?is_use=2" ) )[ "dataList" ]
    for i in tmp : 
        i[ "confirmed" ] = "N"
        i[ "outer_collected" ] = "Y"
    ...
    box_collecting_list  = [ i[ "channel" ] for i in json.loads( url_req( "http://47.96.182.117/index/getBoxChannelList?is_main=2" ) )[ "dataList" ] ]
...
def stream_name_map_get( ch_info_list ): 
    ...
    for source_id  in req_para :
        ...
        req_return = url_req( "http://47.96.182.117/manage/index/getChannelSourceName" , post_str = para  )
...
def collect_summary( ch ,start_date , end_date ):
    ...
    if ( et - st ) % 86400 > 31  : 
        et = st + 86400 * 31 
    gap_list = json.loads( url_req( "http://az.hz-data.xyz:8087/channel/%s/m3u8.gap?start_time=%s&end_time=%s" % ( ch ,st ,et  ) ) )[ "data" ] 
    res = []
...
def vs_gap_summary( ch , s_date ,e_date ):
    ...
    gap_info = json.loads( url_req(   "http://172.16.238.171:8087/channel/%s/m3u8.gap?start_time=%s&end_time=%s" % ( ch ,st,et ) )  )[ "data" ]
#    gap_info = json.loads( url_req(   "http://az.hz-data.xyz:8087/channel/%s/m3u8.gap?start_time=%s&end_time=%s" % ( ch ,st,et ) )  )[ "data" ]
    res = []
    for i in range( st ,et , 86400 ) :
...
def get_score_of_channel( ch ):
    ...
    for i in range( len(  time_gap_list ) - 1 ) :
        ...
        tmp =  url_req( "http://az.hz-data.xyz:8087/channel/%s/playback.m3u8?start_time=%s&end_time=%s" % ( ch , cur_st , cur_et  ) )
